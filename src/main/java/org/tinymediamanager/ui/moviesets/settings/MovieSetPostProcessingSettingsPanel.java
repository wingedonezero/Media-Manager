...
/**
 * the class {@link MovieSetPostProcessingSettingsPanel} holds the settings for post process action for movie sets
 *
 * @author Manuel Laggner
 */
public class MovieSetPostProcessingSettingsPanel extends JPanel {
  ...
  MovieSetPostProcessingSettingsPanel() {
    ...
    btnAddProcess.addActionListener(e -> {
      ...
      PostProcessDialog dialog = new MovieSetPostProcessDialog();
      dialog.pack();
    ...
    btnRemoveProcess.addActionListener(e -> {
      ...
      for (int indexRow : indexRows) {
        try {
          PostProcess process = settings.getMovieSetPostProcess().get(indexRow);
          settings.removeMovieSetPostProcess(process);
        }
        catch (Exception ex) {
          // do nothing
        }
    ...
    btnEditProcess.addActionListener(e -> {
      ...
      if (row != -1) {
        PostProcess process = settings.getMovieSetPostProcess().get(row);
        if (process != null) {
          PostProcessDialog dialog = new MovieSetPostProcessDialog();
  ...
  protected void initDataBindings() {
    BeanProperty<MovieSettings, List<PostProcess>> settingsBeanProperty = BeanProperty.create("movieSetPostProcess");
