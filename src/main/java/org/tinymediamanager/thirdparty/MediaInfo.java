 ...
 *  
 *  Used example code from: https://github.com/MediaArea/MediaInfoLib/blob/master/Source/MediaInfoDLL/MediaInfoDLL.JNA.java
 */
...
public class MediaInfo implements AutoCloseable {
  ...
  public Map<StreamKind, List<Map<String, String>>> snapshot() {
    ...
    for (StreamKind streamKind : StreamKind.values()) {
      int streamCount = Count_Get(streamKind);
      if (streamCount > 0) {
  ...
  public Map<String, String> snapshot(StreamKind streamKind, int streamNumber) {
    ...
    for (int i = 0, count = Count_Get(streamKind, streamNumber); i < count; i++) {
  ...
  public void close() throws Exception {
    try {
      ...
      LOGGER.warn("Error during MediaInfo close - {}", e.getMessage());
    }
  }
}