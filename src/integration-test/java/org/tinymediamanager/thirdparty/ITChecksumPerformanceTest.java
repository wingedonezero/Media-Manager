 ...
 * And a multi-digest test, to see speed differences
 */
public class ITChecksumPerformanceTest { // NOSONAR squid:S5786
  ...
  public void testDifferentCRCs() {
    System.out.println("BufferSize: " + BUFFER_SIZE / 1024 / 1024 + "mb");

    // barely difference between file channels and default stream (from M2 SSD - test on NAS!!!)
    testChecksumImplFileChannel(BIG_FILE, new CRC32()); // ~2 sec
    testChecksumImplFileChannel(BIG_FILE, new CRC32AWS()); // ~5 sec
    testChecksumImplFileChannel(BIG_FILE, new PureJavaCrc32()); // ~5 sec
    testChecksumImplFileChannel(BIG_FILE, new CRC32Apple()); // ~15 sec
    testChecksumImplFileChannel(BIG_FILE, new CRC32Lookup()); // ~15 sec

    testChecksumImpl(BIG_FILE, new CRC32());
    testChecksumImpl(BIG_FILE, new CRC32AWS());
    testChecksumImpl(BIG_FILE, new PureJavaCrc32());
    testChecksumImpl(BIG_FILE, new CRC32Apple());
    testChecksumImpl(BIG_FILE, new CRC32Lookup());
  }

  @Test
  public void testRawAlgoPerf() throws NoSuchAlgorithmException, DigestException {
    System.out.println("BufferSize: " + BUFFER_SIZE / 1024 / 1024 + "mb");

    // md5 & sha384 are extremely slow.
    // crc32 is so simple, it barely takes any CPU!
    Checksum crc32 = new CRC32(); // ~0.2 sec (!)
    MessageDigest md5 = MessageDigest.getInstance("MD5"); // ~10 sec
    MessageDigest sha1 = MessageDigest.getInstance("SHA-1"); // ~3 sec
    MessageDigest sha256 = MessageDigest.getInstance("SHA-256");// ~4 sec
    MessageDigest sha384 = MessageDigest.getInstance("SHA-384"); // ~12 sec

    StopWatch crc32w = new StopWatch();
    StopWatch md5w = new StopWatch();
    StopWatch sha1w = new StopWatch();
    StopWatch sha256w = new StopWatch();
    StopWatch sha384w = new StopWatch();
    crc32w.start();
    crc32w.suspend();
    md5w.start();
    md5w.suspend();
    ...
    try (InputStream inputStream = Files.newInputStream(BIG_FILE, StandardOpenOption.READ)) {
      int bytesRead = 0;
      w.start();
  ...
  private void testChecksumImpl(Path file, Checksum crc32) {
    StopWatch w = new StopWatch();
    byte[] buffer = new byte[BUFFER_SIZE];
    ...
    System.out.print("[" + Long.toHexString(crc32.getValue()).toUpperCase(Locale.ROOT) + "]");
    System.out.print("- took: " + w);
    System.out.println(" - " + crc32.getClass().getName());
  }

  private void testChecksumImplFileChannel(Path file, Checksum crc32) {
    StopWatch w = new StopWatch();
    ...
    try (FileChannel fc = FileChannel.open(file, StandardOpenOption.READ)) {
      ...
      int bytesRead = 0;
      w.start();
      while ((bytesRead = fc.read(bb)) >= 0) {
        bb.flip();
        bb.get(tmp, 0, bytesRead);
        crc32.update(tmp, 0, bytesRead);
      }
      w.stop();
    }
    catch (IOException e) {
  ...
  public final class CRC32AWS implements Checksum, Cloneable {
    ...
    public void update(byte[] b, int offset, int len) {
      ...
      for (int end = offset + len - remainder; i < end; i += 8) {
        int x = localCrc ^ ((((b[i] << 24) >>> 24) + ((b[i + 1] << 24) >>> 16)) + (((b[i + 2] << 24) >>> 8) + (b[i + 3] << 24)));

        localCrc = ((T[((x << 24) >>> 24) + 0x700] ^ T[((x << 16) >>> 24) + 0x600]) ^ (T[((x << 8) >>> 24) + 0x500] ^ T[(x >>> 24) + 0x400]))
   ...
   * </p>
   * <p>
   * Copied from Apache Hadoop 2.6.3. The code agrees with the following file in the 2.6.3 tag:
   * https://gitbox.apache.org/repos/asf?p=hadoop.git;a=blob_plain;
   * f=hadoop-common-project/hadoop-common/src/main/java/org/apache/hadoop/util/PureJavaCrc32.java; hb=2120de588b92b9f22b1cc4188761d6a8c61aa778
   * </p>
   * <p>
  ...
  public class CRC32Apple implements Checksum {
  ...
  public class CRC32Lookup implements Checksum {
    /**
     ...
     */
    private static final int[] CRC_TABLE = { 0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
